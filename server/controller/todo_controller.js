...
export const updateTask = async (req, res) => {
  try {
    ...
    if(!update) {
     ...
     res.status(200).json({message: 'task updated successfull', success: true });
  } catch (error) {
    res.status(500).json({ error: error.message, success: false });
  }
};
export const deleteTask = async (req, res) => {
    ...
    res.status(203).json({message: 'your data has been deleted', deletedData: delData, success: true})
}